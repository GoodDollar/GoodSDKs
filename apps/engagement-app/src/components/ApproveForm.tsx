...
const ApproveForm: React.FC = () => {
  ...
  const [previousDetails, setPreviousDetails] = useState<AppDetails | null>(null);
  ...
  useEffect(() => {
    const loadAppDetails = async () => {
      ...
      const events = await engagementRewards.getAppHistory(appAddress as `0x${string}`);
      if (events.length > 1) {
          ...
          <div className="space-y-6">
            {currentDetails && (
              <div className="space-y-4">
                <h3 className="text-lg font-medium">Current Application Details</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>Owner: <TruncatedAddress address={currentDetails.owner} /></div>
                  <div>Receiver: <TruncatedAddress address={currentDetails.rewardReceiver} /></div>
                  <div>User & Inviter: {currentDetails.userAndInviterPercentage}%</div>
                  <div>User: {currentDetails.userPercentage}%</div>
                  <div>URL: <a href={currentDetails.url} target="_blank" rel="noopener noreferrer">{currentDetails.url}</a></div>
                  <div>Email: <a href={`mailto:${currentDetails.email}`}>{currentDetails.email}</a></div>
                  <div className="col-span-2">Description: {currentDetails.description}</div>
                </div>
              </div>
            )}
            ...
            {previousDetails && (
              <div className="space-y-4 mt-8 pt-8 border-t">
                <h3 className="text-lg font-medium">Previous Approved Details</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>Owner: <TruncatedAddress address={previousDetails.owner} /></div>
                  <div>Receiver: <TruncatedAddress address={previousDetails.rewardReceiver} /></div>
                  <div>User & Inviter: {previousDetails.userAndInviterPercentage}%</div>
                  <div>User: {previousDetails.userPercentage}%</div>
                  <div>URL: <a href={previousDetails.url} target="_blank" rel="noopener noreferrer">{previousDetails.url}</a></div>
                  <div>Email: <a href={`mailto:${previousDetails.email}`}>{previousDetails.email}</a></div>
                  <div className="col-span-2">Description: {previousDetails.description}</div>
                </div>
              </div>
            )}
