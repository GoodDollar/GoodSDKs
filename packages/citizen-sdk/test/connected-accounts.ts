...
import { chainConfigs, SupportedChains, type contractEnv } from './src/constants'
...
async function runTests() {
    ...
    try {
        ...
        const { isWhitelisted, root } = await identitySDK.getWhitelistedRoot(config.mainAccount)

        const isSelf = root.toLowerCase() === config.mainAccount.toLowerCase()
        if (isWhitelisted && isSelf) {
            logTest('Main account correctly resolves to itself', true, `Root: ${root}`)
            passedTests++
        } else {
            logTest('Main account resolution failed', false, `Whitelisted: ${isWhitelisted}, Root: ${root}`)
        }
    } catch (error: any) {
    ...
    try {
        ...
        const { isWhitelisted, root } = await identitySDK.getWhitelistedRoot(config.connectedAccount)
        ...
        if (isWhitelisted && isConnected) {
            ...
            logTest('Connected account resolution failed', false, `Whitelisted: ${isWhitelisted}, Root: ${root}`)
        }
    } catch (error: any) {
    ...
    try {
        ...
        if (match) {
            logTest('ClaimSDK correctly throws descriptive error for non-whitelisted', true, `Caught: ${error.message}`)
            passedTests++
        } else {
            logTest('ClaimSDK threw unexpected error', false, `Expected descriptive error, got: ${error.message}`)
        }
    }
    ...
    try {
        ...
        logTest(`Status successfully retrieved: ${status.status}`, true, `Entitlement: ${status.entitlement}`)
        passedTests++
    } catch (error: any) {
